...
AddModule(function()
	...
	maryo.Config = function(parent: GuiBase2d)
		Util_CreateDropdown(parent, "Cap", {
			...
			"Metal Mario",
			"Vanish Cap Mario",
		}, maryo.ModeCap).Changed:Connect(function(val)
			maryo.ModeCap = val
		end)
	end
	maryo.LoadConfig = function(save: any)
		maryo.FPS30 = not not save.FPS30
		maryo.ModeCap = save.ModeCap or maryo.ModeCap
	end
	maryo.SaveConfig = function()
		return {
			FPS30 = maryo.FPS30,
			ModeCap = maryo.ModeCap,
		}
	end
	...
	do -- MOVING
		...
		local function checkLedgeClimbDown()
			if maryo.ForwardVel < 10 then
				local pos, wall = Util.FindWallCollisions(maryo.Position, -10, 10)
				if wall then
					...
					if floor and pos.Y - floorHeight > 160 then
						...
						local wallDYaw = wallAngle - maryo.FaceAngle.Y
						if math.abs(wallDYaw) < 0x4000 then
							pos -= Vector3.new(20 * wall.Normal.X, 0, 20 * wall.Normal.Z)
							maryo.Position = pos
							maryo.FaceAngle *= Vector3int16.new(0, 1, 1)
							maryo.FaceAngle = maryo.SetFaceYaw(wallAngle + 0x8000)
		...
		local function slideBonk(, fastAction: number, slowAction: number)
		...
		local function updateSlidingAngle(, accel: number, lossFactor: number)
			local newFacingDYaw
			local facingDYaw
			...
			maryo.SlideVelX += accel * steepness * Util.Sins(slopeAngle)
			maryo.SlideVelZ += accel * steepness * Util.Coss(slopeAngle)
			maryo.SlideVelX *= lossFactor
			maryo.SlideVelZ *= lossFactor
			maryo.SlideYaw = Util.Atan2s(maryo.SlideVelZ, maryo.SlideVelX)
			facingDYaw = Util.SignedShort(maryo.FaceAngle.Y - maryo.SlideYaw)
		...
		local function updateSliding(, stopSpeed: number)
		...
		local function animAndAudioForWalk()
			...
			maryo.GfxAngle = Vector3.new(walkingPitch, maryo.GfxAngle.Y, maryo.GfxAngle.Z)
		end
		local function pushOrSidleWall(, startPos: Vector3)
			...
			if wall == nil or math.abs(dWallAngle) >= 0x71C8 then
				...
				maryo.GfxAngle = Vector3.new(maryo.GfxAngle.X, maryo.ActionArg, maryo.GfxAngle.Z)
				maryo.GfxAngle = Vector3.new(maryo.GfxAngle.X, maryo.GfxAngle.Y, maryo.FindFloorSlope(0x4000))
			end
		end
	...
	maryo.Reset()
	maryo.Init = function(figure: Model)
		local root = figure:FindFirstChild("HumanoidRootPart")
		maryo.Reset(root and root.Position or nil)
	end
	maryo.Update = function(dt: number, figure: Model)
		local t = os.clock()
		if maryo.ModeCap == 1 then
	...
	maryo.Destroy = function(figure: Model?)
	end
	return m
end)
return modules